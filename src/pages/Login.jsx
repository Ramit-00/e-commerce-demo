...
const submitHandler = async(e) => {
  ...
  try{
    ...
    if(res.data.success){
      navigate('/')
      dispatch(setUser(res.data.user))  // Update the user state in the Redux store with the logged-in user's information
      localStorage.setItem("accessToken", res.data.accessToken);  // Store the access token in local storage for future authenticated requests
      ...
      <Card className="w-full max-w-sm">
        ...
        <CardFooter className="flex-col gap-2">
          <Button onClick={submitHandler} type="submit" className="w-full cursor-pointer bg-black hover:bg-gray-600">
            {loading ? <><Loader2 className="w-5 h-5 animate-spin" mr-2 /> Please wait </> : "Login"}
          </Button>
          <p className='text-gray-700 text-sm'> Don't have an account? <Link className='hover:underline cursor-pointer text-pink-800' to='/signup'> Signup </Link></p>
        </CardFooter>
      </Card>
    </div>
  )
